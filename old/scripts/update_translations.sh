...
( cd $LOCAL_MIRROR
for jPath in "$TRANSLATIONS"/slintwebsite.templatepot/*; do
  [ -e "$jPath" ] || continue
  j=`basename "$jPath"`
  ll=`echo "$j"|awk -F"." '{print $2}'`
  if [ ! -f po/template/$j ] || [ ! "`diff "$jPath" po/template/$j`" = "" ] || [ ! -f slint.conf.$ll ]; then
    cp -f "$jPath" po/template
...
for PAGE in $PAGES; do
  for TRANSLATION_PO_PATH in "$TRANSLATIONS"/slintwebsite."${PAGE}"pot/*; do
    [ -e "$TRANSLATION_PO_PATH" ] || continue
    TRANSLATION_PO=`basename "$TRANSLATION_PO_PATH"`
    if [ ! -f po/$PAGE/$TRANSLATION_PO ] || \
    [ ! "`diff po/$PAGE/$TRANSLATION_PO "$TRANSLATION_PO_PATH"`" = "" ]; then
      ...
      cp "$TRANSLATION_PO_PATH" po/$PAGE
